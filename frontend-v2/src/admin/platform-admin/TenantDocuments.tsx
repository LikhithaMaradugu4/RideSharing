...
const TenantDocuments = () => {
  ...
  const loadDocuments = async () => {
    try {
      setLoading(true);
      const data: TenantDocument[] = await adminService.listTenantDocuments(Number(tenantId));
      setDocuments(data);
      setError('');
    } catch (err: any) {
  ...
  const handleDownload = async (documentId: number, fileName: string) => {
    try {
      const { blob, filename } = await adminService.downloadTenantDocument(Number(tenantId), documentId);
  ...
  const handleDelete = async (documentId: number, fileName: string) => {
    ...
    try {
      await adminService.deleteTenantDocument(Number(tenantId), documentId);
      loadDocuments();
    } catch (err: any) {
